<script>
  ...
  let msg = window.fms.msg;
  let msgFrom = window.fms.msgFrom;
  let mode = window.fms.mode;
  let isDisabled = false;

  function handleChangeMsg(){
    window.fms.changeMsg('Hello from Svelte');
  };
  function handleChangeMsgFrom(){
    window.fms.changeMsgFrom('svelte');
  };
  function handleChangeMode(e){
      window.fms.changeMode('svelte');
  };
...
<div class="fms-m-card-app {mode === 'init' || mode === 'svelte' ? '-mode-svelte' : '-mode-default'}"
>
    <h2 class="card-header">Svelte.js</h2>
    <figure class="svelte-logo">
    ...
    <div class="fms-c-btn -small card-send" on:click={handleChangeMsg} on:click={handleChangeMsgFrom} >
      Send meassage
    </div>
    ...
    <div class="fms-c-btn -xsmall -ghost card-theme" class:-disabled={isDisabled} on:click={handleChangeMode} >
      Apply theme
    </div>
</div>
